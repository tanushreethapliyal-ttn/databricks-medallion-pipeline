"""Bootstrap sys.path for Gold notebooks on Databricks."""
...
def setup_paths() -> tuple[str, str]:
    """Add gold and bronze directories to sys.path; return (gold_dir, bronze_dir)."""
    gold_dir = resolve_gold_dir()
    repo_root = str(Path(gold_dir).resolve().parents[1])
    bronze_dir = os.path.join(repo_root, "src", "bronze")

    for path in (gold_dir, bronze_dir):
        if path not in sys.path:
            sys.path.insert(0, path)

    return gold_dir, bronze_dir
