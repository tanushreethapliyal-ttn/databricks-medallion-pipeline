...
# MAGIC Raw ingest of `products.csv` into `bronze.products` (no transformations).
...
from schemas import PRODUCTS_BRONZE_SCHEMA
...
def run(run_id: str | None = None) -> int:
    ...
    row_count = ingest_entity(
        spark, config, "products", PRODUCTS_BRONZE_SCHEMA, effective_run_id
    )
    print(f"SUCCESS: bronze.products loaded with {row_count} rows (run_id={effective_run_id})")
    return row_count
