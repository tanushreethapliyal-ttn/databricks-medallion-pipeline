...
# MAGIC %md
# MAGIC # Bronze — Ingest Customers
# MAGIC Raw ingest of `customers.csv` into `bronze.customers` (no transformations).
...
from schemas import CUSTOMERS_BRONZE_SCHEMA
...
def run(run_id: str | None = None) -> int:
    ...
    row_count = ingest_entity(
        spark, config, "customers", CUSTOMERS_BRONZE_SCHEMA, effective_run_id
    )
    print(f"SUCCESS: bronze.customers loaded with {row_count} rows (run_id={effective_run_id})")
    return row_count
